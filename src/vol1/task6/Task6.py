...
class Task6_vol1(Task):
    ...
    def get_info(self):
        cond = ("Среди максимальных путей в дереве выбрать тот, у которого сумма ключей всех его вершин максимальна, "
                "затем удалить центральную вершину и корень этого пути (правым удалением). "
                "Выполнить прямой (левый) обход полученного дерева." )
        return {
            "title": "Максимальный путь с наибольшей суммой ключей. Удаление",
            "condition": cond,
            "input": "Массив целых чисел - узлы дерева",
            "output": "Строка - ключи дерева, записанные прямым левым обходом"
        }